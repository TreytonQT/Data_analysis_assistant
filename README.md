...
不需要配置 Python、Node.js 或 Docker，可直接从 [GitHub Releases](https://github.com/TreytonQT/Data_analysis_assistant/releases/latest) 下载 `DataAnalysisAssistant-windows-x64-v2.1.0.zip`：
...
- **销量看板**：按店铺和产品等级分析在售数、订单、日均销量及库存表现，并支持服务端排序和完整 CSV 导出。
- **滞销提醒**：计算库龄库存、占用资金、计提与弃置费用；关联库存图片、日均销量和最近一次促销快照，按动销策略管理促销候选与促销周期。
- **产品管理**：合并运营、毛利、Rating、批次、图片和当前促销数据，以决策矩阵查看 SKU 级产品表现与低毛利异常。
- **部门监控**：提供业绩、提成和考核三个视图；考核监控按月份和开发员汇总中企/本土销售额，并支持调整本土业绩折扣比例。
- **补货管理**：以 ASIN 为主行展示德/法/西/意站点毛利、库存矩阵、趋势测算、12 个月销量画像、库存图片和建议补货数量；支持搜索、开发员批量筛选、最低补货量、排序、分页、Excel 导出、产品标签和带原因的不补货操作。
- **批次监控**：创建和导入批次，维护美工图、上架售价、货件绑定与到货状态，并识别未绑定或未归属批次的 SKU。
- **上传中心**：按每日、每周、每月分组管理业绩报表、运营原始表、毛利原始表、Rating、SKU 图片映射表、销量/销售额明细和滚动 12 个月销量原始表，支持预览、下载、替换和删除。
- **配置中心**：维护指标公式、店铺、目标、提成、部门费用率、库存覆盖规则、补货开关和 ASIN 产品标签。
- **待办提醒**：创建、排序、流转、重复提醒、导入和导出本地待办事项。

界面使用多级菜单和最近访问标签页组织业务视图，支持浏览器前进/后退恢复页面与筛选状态。保存、上传、下载、复制、删除等操作会显示统一的进行中、成功或失败反馈。
...
启动脚本会调用独立桌面启动器，检查 Python 依赖、React 构建产物和 `8000` 端口，在后台启动 FastAPI，等待 `/api/health` 就绪后再打开浏览器。运行日志和进程号保存在 `.tmp/`。
...
首次部署后，可运行 powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\install_desktop_shortcut.ps1 创建桌面“销售数据看板”快捷方式。该快捷方式启动的服务不依赖 Codex 或启动终端保持打开。

看板右上角提供“重启系统”和“退出系统”。重启会恢复当前页面，退出后请使用桌面快捷方式重新启动。
